...
{
template<class T>
class ClassDescriptor : public RED4ext::TTypedClass<T>, public detail::TypeDescriptor<T>
{
public:
    ClassDescriptor() : RED4ext::TTypedClass<T>({})
    {
    }
    ...
    RED4ext::CClassStaticFunction* AddFunction(StaticFunction<R, TR> aFunc, const char* aName,
    ...
    RED4ext::CClassStaticFunction* AddFunction(MemberFunction<C, R, TR> aFunc, const char* aName,
                                               RED4ext::CBaseFunction::Flags aFlags = {})
    {
        const auto* ptr = reinterpret_cast<RED4ext::ScriptingFunction_t<R*>>(aFunc);
...
private:
    ...
    inline static Registrar s_registrar{ &OnRegisterRTTI, &OnPostRegisterRTTI }; // NOLINT(cert-err58-cpp)
};

template<class T, class P = RED4ext::IScriptable>
requires detail::IsScripable<P>
class Class : public P
{
public:
    using Descriptor = ClassDescriptor<T>;
    using Parent = P;
